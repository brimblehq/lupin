...
function NavGroup({ label, items, active }: { label: string; items: readonly GroupItem[]; active: boolean }) {
          ...
          {items.map((it) => {
            ...
            const inner = (
              <>
                ...
                {it.description && <span className="font-body text-[12px] leading-[16px] text-brimble-black/50">{it.description}</span>}
              </>
            );
            return it.external ? (
              <a
  ...
  return (
    ...
    >
      ...
      >
        {label}
        <ChevronDown className={`size-3 transition-transform duration-200 ${open ? "rotate-180" : "rotate-0"}`} aria-hidden="true" />
      </button>
